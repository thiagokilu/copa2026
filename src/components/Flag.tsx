...
const Flag = ({ name }: FlagProps) => {
  ...
  if (!code || code === "eu" || code === "un") {
    return (
      <div
        className="w-9 h-6 rounded flex items-center justify-center text-sm flex-shrink-0"
  ...
  return (
    <img
      src={FLAG_URL(code)}
      alt={name}
      className="w-9 h-6 object-cover rounded flex-shrink-0 border border-white/20"
